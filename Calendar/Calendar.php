...
class Calendar {
    ...
    public function __construct($config = array()) {
        ...
        define("LOC", $this->loc); // 
        $this->lang = include 'Lang.php'; //language file
    }
    ...
    public function initialize($config = array()) {
        ...
        if ($this->show_next_prev === TRUE && empty($this->next_prev_url)) {
            $uri = explode('/', parse_url($_SERVER['QUERY_STRING'], PHP_URL_PATH));
            $uri = $uri[0];
            $this->next_prev_url =  $uri;
        }
    ...
    public function generate($year = '', $month = '', $data = array()) {
        ...
        if ($this->show_next_prev === TRUE) {
            ...
            $out .= str_replace('{previous_url}', DIR . $this->next_prev_url . $adjusted_date['year'] . '/' . $adjusted_date['month'], $this->replacements['heading_previous_cell']) . "\n";
        }
        ...
        if ($this->show_next_prev === TRUE) {
            $adjusted_date = $this->adjust_date($month + 1, $year);
            $out .= str_replace('{next_url}', DIR . $this->next_prev_url . $adjusted_date['year'] . '/' . $adjusted_date['month'], $this->replacements['heading_next_cell']);
        }
    ...
    public function get_month_name($month) {
        ...
        return ($this->lang[$month_names[$month]] === FALSE) ? ucfirst(substr($month_names[$month], 4)) : $this->lang[$month_names[$month]];
    }
    ...
    public function get_day_names($day_type = '') {
        ...
        for ($i = 0, $c = count($day_names); $i < $c; $i++) {
            $days[] = ($this->lang['cal_' . $day_names[$i]] === FALSE) ? ucfirst($day_names[$i]) : $this->lang['cal_' . $day_names[$i]];
        }
